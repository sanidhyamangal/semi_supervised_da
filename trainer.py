...
class BaseTrainer:
    def __init__(self, model:tf.keras.Model,optimizer:tf.keras.optimizers.Adam, log_file_name:str="logs.csv", num_classes:int=65) -> None:
        self.model = model
    ...
    def train_step(self, source_dataset, target_dataset, unlabelled_dataset):
        ...
        with tf.GradientTape() as tape:
            for batch_source_images, batch_source_labels in source_dataset:
                ...
                _src_one_hot_labels = tf.one_hot(batch_source_labels, depth=self.num_classes)
            ...
            for batch_target_images, batch_target_labels in target_dataset:
                ...
                _tgt_one_hot_labels = tf.one_hot(batch_target_labels, depth=self.num_classes)
                _tgt_loss = compute_h(_tgt_one_hot_labels, _tgt_preds)
                _target_loss.append(_tgt_loss)
            
            for batch_unlabelled_images, batch_perturbed_images in unlabelled_dataset:
            ...
            loss = tf.reduce_mean(_source_loss) + tf.reduce_mean(_target_loss) + tf.reduce_mean(_cr_loss)
    ...
    def train(self, epochs:int, source_dataset, target_dataset, unlabelled_dataset,weights_path="str"):
        for i in epochs:
            # call training step
            loss = self.train_step(source_dataset, target_dataset, unlabelled_dataset)
